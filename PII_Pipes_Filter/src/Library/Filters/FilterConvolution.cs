...
namespace CompAndDel.Filters
{
    ...
    public class FilterConvolution : IFilter
    {
        ...
        protected ConvultionType filter;
        ...
        /// </summary>
        public FilterConvolution(ConvultionType filter)
        {
            this.kernel = filter.kernelarray;
            this.complement = filter.complement;
            this.divider = filter.divider;
            for (int x = 0; x < filter.kernel; x++)
            {
                for (int y = 0; y < filter.kernel; y++)
                {
                    kernel[x, y] = 1;
                }
            }
